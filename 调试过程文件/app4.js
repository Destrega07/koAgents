...
function sliceNeedInsightMarkdown(markdown) {
  ...
  const toSlotsWithTail = (block) => {
    ...
    return { slots: items.map((t) => ({ text: t, checked: false })), tail };
  };
...
async function renderNeedInsight() {
  ...
  const rawText =
    state.ui.needInsightRaw.get(cid) ||
    (typeof stored?.rawStream === "string" ? stored.rawStream : "") ||
    (typeof stored?.raw === "string" ? stored.raw : report);
