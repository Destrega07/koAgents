...
function sliceValuePropMarkdown(markdown) {
  ...
  const plansBlock = pickSection("方案及价值主张罗列");
  const uniqBlock = pickSection("独特性分析");
  const tableBlock = pickSection("(?:总结表格|价值效率与投资回报评估)");
  const synthBlock = pickSection("价值主张合成");
  ...
  const flush = () => {
    ...
    blocksByGroup.push({ group: curGroup, block: curLines.join("\n").trim() });
    curLines = [];
  };
  for (const line of String(plansBlock || "").split("\n")) {
  ...
  const groupsToParse = blocksByGroup.length ? blocksByGroup : [{ group: "", block: String(plansBlock || "").trim() }];
  for (const { group, block } of groupsToParse) {
    const items = parseListItems(block);
    for (const it of items) {
      const m = String(it || "").match(/^\s*(方案[一二三四五六七八九十0-9]+)(?:\s*（[^）]*）|\s*\([^)]*\))?\s*[：:]\s*([\s\S]*)$/);
      if (!m) continue;
      const scheme = String(m[1] || "").trim();
      const rest = String(m[2] || "").trim();
      plans.push({ group: String(group || ""), scheme, text: rest ? `${scheme}：${rest}` : scheme, checked: false });
    }
  }
...
async function runValuePropPipeline(customerId, pipeline_context) {
  ...
  try {
    ...
    const { raw, lastJson, dataLines } = await fetchCozeChatStream(reqBody, { signal: controller.signal });
    ...
    const sliced = sliceValuePropMarkdown(report);
...
async function renderValueProp() {
  ...
  if (!report || !sliced) {
    ...
    goBtn.onclick = async () => {
      ...
      const picked = (Array.isArray(sliced.slots?.plans) ? sliced.slots.plans : [])
        ...
        .map((p) => ({ group: String(p.group || ""), text: String(p.text || "") }));
      await setModuleData(cid, "value_prop", { ...(stored || {}), selectedPlans: picked });
      const content =
        `# 已选方案清单\n\n` +
        picked.map((p, i) => `${i + 1}. ${p.group ? `[${p.group}] ` : ""}${p.text}`).join("\n") +
        `\n\n---\n\n## 方案独特性分析\n\n${String(sliced.blocks?.uniqBlock || "")}\n\n---\n\n## 价值主张合成\n\n${String(
          sliced.blocks?.synthBlock || "",
        )}`;
      await setModuleData(cid, "proposal", { content });
  ...
  const rawStream = String(state.ui.valuePropRaw.get(cid) || stored?.rawStream || "").trim();
  const pre = document.createElement("pre");
  pre.className = "raw-pre";
  pre.textContent =
    (report ? `【解析清洗后的 Markdown 报告】\n\n${String(report)}\n\n` : "【解析清洗后的 Markdown 报告】\n\n暂无内容\n\n") +
    `---\n\n【流式原始 rawStream】\n\n${rawStream || "暂无内容"}`;
  details.appendChild(summary);
  details.appendChild(pre);
  panel.appendChild(details);
