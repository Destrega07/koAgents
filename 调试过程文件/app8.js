...
async function streamCozeAnswerText(payload, { signal, onDelta } = {}) {
  ...
  return { raw, mergedDelta, completed, aborted: false };
}
...
async function renderProposal() {
  ...
  exportTxt.onclick = async () => {
    ...
    const text = (draft && typeof draft.content === "string" ? draft.content : "") || (latest && latest.content ? String(latest.content) : "");
  ...
  exportPng.onclick = () => toast("导出长图组件正在封装中...");
  ...
  const content = loading ? draftContent || (storedContent === "生成中…" ? "" : storedContent) : draftContent || storedContent;
