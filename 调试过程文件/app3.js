...
async function renderNeedInsight() {
  ...
  if (!report) {
    ...
    const supportSection = renderNeedInsightSlotSection("支持需求", sliced.slots?.support);
    const outcomeSection = renderNeedInsightSlotSection("成果需求", sliced.slots?.outcome);
    const personalSection = renderNeedInsightSlotSection("个人需求", sliced.slots?.personal);
    ...
    goBtn.onclick = async () => {
      ...
      addChecked(sliced?.slots?.support);
      addChecked(sliced?.slots?.outcome);
      addChecked(sliced?.slots?.personal);

      const payload = {
        basicInfo: String(sliced?.blocks?.basicInfo || ""),
